...
def alt_hpo():
    path='/root/autodl-tmp/try-hposy-e1/dict/'
    fin=open(path+'obo.json','r',encoding='utf-8')
...
def document_metric(pre_result, gold_result):
    ...
    #print(microP,microR,microF,macroP,macroR,macroF)
    return macroF
...
def mention_metric_new(pre_result, gold_result):
    ...
    #print(NER_P_r,NER_R_r,NER_F_r,NEN_P_r,NEN_R_r,NEN_F_r)
    return NEN_F_r
...
def new_read_files_in_directory(directory):
    all_pre = []
    ...
    for filename in os.listdir(directory):
        ...
        if os.path.isfile(filepath):
            ...
            with open(filepath, 'r') as fin:
                ...
                all_pre.append(content_with_numbers)
...
def GSCplus_corpus_hponew(prefile,goldfile,subtree=True):
    ...
    all_gold=new_read_files_in_directory(goldfile)
    pre_result={}
    gold_result={}
    for doc_pre in all_pre:
        lines=doc_pre.split('\n')
...
if __name__=='__main__':
    path='/root/autodl-tmp/try-hposy-e1/'
    GSCplus_corpus_hponew(path+'example/output/ID-68',path+'tagger_goldeva/id68_gold',subtree=True)
# 装答案的文件夹os一个个读取